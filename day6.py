...
def toUnionCount(answers):
	return len(set.union(*map(set, answers.split('\n'))))

def toIntCount(answers):
	return len(set.intersection(*map(set, answers.split('\n'))))

if __name__ == '__main__':
	print('Part 1: ', sum(map(toUnionCount, input.split('\n\n'))))
	print('Part 2: ', sum(map(toIntCount, input.split('\n\n'))))