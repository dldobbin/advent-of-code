...
for rule in input:
	...
	m = re.search('^(.*) bags$', container)
	key = m.group(1)
	value = re.findall('(\d) (.*?) bag', containees)
	bagMap[key] = [bag for num, bag in value for i in range(int(num))]

def bagsThatContain(bag):
	return [k for k,v in bagMap.items() if bag in v]
