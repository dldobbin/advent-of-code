...
def isStricterValid(passport):
	...
	try:
		return ((1920 <= int(map['byr']) <= 2002) and
			(2010 <= int(map['iyr']) <= 2020) and
			(2020 <= int(map['eyr']) <= 2030) and
			((map['hgt'][-2:] == 'cm' and 150 <= int(map['hgt'][:-2]) <= 193) or
			 (map['hgt'][-2:] == 'in' and 59 <= int(map['hgt'][:-2]) <= 76)) and
			re.match('^#[a-f0-9]{6}$', map['hcl']) and
			re.match('amb|blu|brn|gry|grn|hzl|oth', map['ecl']) and
			re.match('^\d{9}$', map['pid']))
	except KeyError:
		return False

if __name__ == '__main__':
	print('Part 1: ', len([passport for passport in input.split('\n\n') if isValid(passport)]))
	print('Part 2: ', len([passport for passport in input.split('\n\n') if isStricterValid(passport)]))